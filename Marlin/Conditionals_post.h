...
#define CONDITIONALS_POST_H
   ...
   */
      ...
      // Boundaries for Cartesian probing based on set limits
        ...
        #define _MESH_MIN_X (max(X_MIN_BED + MESH_INSET + MESH_OFFSET_X, X_MIN_POS))  // UBL is careful not to probe off the bed.  It does not
        #define _MESH_MIN_Y (max(Y_MIN_BED + MESH_INSET + MESH_OFFSET_Y, Y_MIN_POS))  // need *_PROBE_OFFSET_FROM_EXTRUDER in the mesh dimensions
        #define _MESH_MAX_X (min(X_MAX_BED - (MESH_INSET) + MESH_OFFSET_X, X_MAX_POS))
        #define _MESH_MAX_Y (min(Y_MAX_BED - (MESH_INSET) + MESH_OFFSET_Y, Y_MAX_POS))
      #else
